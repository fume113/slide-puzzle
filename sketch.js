  ...
  for (let i = 0; i < slice_w; i++)
    for (let j = 0; j < slice_h; j++) {
      ...
      if (board[index] > -1) {
        ...
        // Menggambar ubin kosong dengan warna hitam
        fill(255, 0, 0); // Merah
        rect(x, y, w, h);
      }
    }
...
// Fitur Button Finish Instant
const finish_instant = document.getElementById("finish_instant");
// Function Play
finish_instant.onclick = function () {
  solvedFlag = true;
  window.location.href = "narasi.html?name=" + encodeURIComponent(name);
};
